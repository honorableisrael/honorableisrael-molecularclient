...
const Experience = withRouter((props) => {
  ...
  const inputModalChange = (state, credential_id) => {
    ...
    Axios.put(`${API}/admin/specialists/experiences/${credential_id}`, data, {
      headers: { Authorization: `Bearer ${token.access_token}` },
    })
      .then((res) => {
  ...
  const displayExperience = () => {
    ...
    Axios.post(`${API}/admin/specialists/${props.match.params.id}/experiences`, data, {
      headers: { Authorization: `Bearer ${token.access_token}` },
    })
      .then((res) => {
          ...
          setState({
            ...
            addexperiencebtn:
              experiences.length >= 0 ? "profcerbtnwrapper" : "nowrapdemacator",
              isloading:false
          });
  ...
  return (
    <>
      <Modal show={deleteCredential} centered={true} onHide={closeDeleteModal}>
        <div className="usermodaltitle">Delete Experience?</div>
        <Modal.Body>
